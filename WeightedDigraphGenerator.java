...
/**
 * The {@code WeightedDigraphGenerator} class provides static methods for creating
...
public class WeightedDigraphGenerator {
	...
	private final double weight;
        ...
        private Edge(int v, int w) {
            this.v = v;
            this.w = w;
	    this.weight = StdRandom.uniformDouble(1,100);
        }
    ...
    public static EdgeWeightedDigraph dag(int V, int E) {
        ...
        while (G.totalEdges < E) {
            ...
            if ((v < w) && !set.contains(e)) {
                set.add(e);
                G.addEdge(vertices[v] + "", vertices[w] + "", e.weight);
            }
        }
        return G;
    }
    ...
    public static EdgeWeightedDigraph completeRootedInDAG(int V) {
		...
		Edge e = new Edge(vertices[i], vertices[j]);
                G.addEdge(vertices[i] + "", vertices[j] + "", e.weight);
	    }
    ...
    public static EdgeWeightedDigraph completeRootedOutDAG(int V) {
        ...
        for (int i = 0; i < V; i++)
            for (int j = i + 1; j < V; j++) {
		Edge e = new Edge(vertices[j], vertices[i]);
                G.addEdge(vertices[j] + "", vertices[i] + "", e.weight);
	    }
    ...
    public static EdgeWeightedDigraph rootedOutDAG(int V, int E) {
        ...
        for (int v = 0; v < V - 1; v++) {
            ...
            Edge e = new Edge(w, v);
            set.add(e);
            G.addEdge(vertices[w] + "", vertices[v] + "", e.weight);
        }
    ...
    public static EdgeWeightedDigraph path(int V) {
	    ...
	    Edge e = new Edge(vertices[i], vertices[i+1]);
            G.addEdge(vertices[i] + "", vertices[i + 1] + "", e.weight);
        }
        return G;
    }
    ...
    public static EdgeWeightedDigraph binaryTree(int V) {
        ...
        for (int i = 1; i < V; i++) {
	    Edge e = new Edge(vertices[i], vertices[i-1]);
            G.addEdge(vertices[i] + "", vertices[(i - 1) / 2] + "", e.weight);
        }
        return G;
    }
	...
	Edge e = new Edge(vertices[V-1], vertices[0]);
        G.addEdge(vertices[V - 1] + "", vertices[0] + "", e.weight);
        return G;
    }
	...
	Edge e = new Edge(vertices[E-1], vertices[0]);
        G.addEdge(vertices[E - 1] + "", vertices[0] + "", e.weight);
        return G;
    }
    ...
    /**
     * Unit tests the {@code WeightedDigraphGenerator} library.
     *
